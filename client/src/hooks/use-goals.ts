...
import { api, buildUrl, type InsertGoal, type UpdateGoalRequest } from "@shared/routes";
...
export function useGenerateDefaultGoals() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async () => {
      const res = await fetch(api.goals.generateDefaults.path, {
        method: api.goals.generateDefaults.method,
        credentials: "include",
      });
      if (!res.ok) throw new Error("Failed to generate default goals");
      return api.goals.generateDefaults.responses[201].parse(await res.json());
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [api.goals.list.path] }),
  });
}
